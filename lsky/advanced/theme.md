...
默认主题是典型的混合部署，利用 laravel 通配符路由（排除`/api`开头的路由）匹配所有 url 请求路由到主题入口，然后通过 blade 优先渲染 spa 单页的入口文件，spa 单页渲染完成后调用 `/api` 开头的相关接口获取数据，从而实现前后端不需要配合 nginx 伪静态即可实现混合部署，解决了 spa 单页在刷新网页时出现 404 的问题。生命周期图：
