...
| AppID | 应用 ID |
| AppKey | 应用密钥 |
| 登录方式 | 选择一种登录方式（QQ、微信、支付宝、微博、百度、华为、小米、抖音、哔哩哔哩、钉钉） |
| 按钮显示名称 | 登录页按钮上的文案，如「QQ 登录」。留空则显示「彩虹聚合登录」 |

在 [彩虹聚合登录](https://u.cccyun.cc/) 注册并创建应用后获取 AppID 和 AppKey。

::: warning
每个彩虹聚合登录插件实例只能配置一种登录方式。如果需要同时提供 QQ 和微信登录，建议 QQ 使用彩虹聚合登录，微信使用系统内置的微信登录（直连体验更好）。
:::
