...
在节点编辑页面，可以独立设置 CPU、内存、磁盘的超开比率：

| 字段 | 说明 | 默认值 | 建议范围 |
|------|------|--------|----------|
| CPU 超开比率 | 可分配 CPU = 物理核数 × 比率 | 1（不超开） | 2~8 |
| 内存超开比率 | 可分配内存 = 物理内存 × 比率 | 1（不超开） | 1~2 |
| 磁盘超开比率 | 可分配磁盘 = 物理磁盘 × 比率 | 1（不超开） | 1~1.5 |

例如，一台 32 核 / 64GB 内存的服务器，设置 CPU 超开 4 倍、内存超开 1.5 倍后，可分配 128 核 CPU 和 96GB 内存。当已分配资源超过超开后的上限时，才会提示「资源不足」。
