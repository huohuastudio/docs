# 支付配置 {#payment}

Novaix 集成了多种支付渠道，支持国内和国际用户的支付需求。所有支付配置都在管理面板的「系统设置」中完成，配置后立即生效，无需重启程序。每个支付渠道都有独立的启用开关，您只需配置并启用需要的渠道即可。

## 支持的支付渠道 {#supported-channels}

### 支付宝 {#alipay}

支持支付宝电脑网站支付，采用**证书模式**配置，需要准备以下信息：

| 字段 | 说明 |
|------|------|
| App ID | 支付宝开放平台应用 ID |
| 应用私钥 | RSA2 应用私钥 |
| 应用公钥证书 | 应用公钥证书内容（`appCertPublicKey_*.crt`） |
| 支付宝公钥证书 | 支付宝公钥证书内容（`alipayCertPublicKey_RSA2.crt`） |
| 支付宝根证书 | 支付宝根证书内容（`alipayRootCert.crt`） |
| 沙箱模式 | 是否使用沙箱环境（测试用） |

::: tip
您需要在[支付宝开放平台](https://open.alipay.com)创建应用，选择**公钥证书**方式签名，下载三个证书文件后将内容填入对应字段。
:::

### 微信支付 {#wechat-pay}

支持微信 Native 支付（扫码支付），需要准备以下信息：

| 字段 | 说明 |
|------|------|
| AppID | 微信公众号或开放平台应用 ID |
| 商户号 | 微信支付商户号 |
| API v3 密钥 | 商户平台设置的 API v3 密钥 |
| 商户证书序列号 | 商户 API 证书序列号 |
| 商户私钥 | 商户 API 私钥 |
| 微信支付公钥 ID | 微信支付平台公钥 ID |
| 微信支付公钥 | 微信支付平台公钥内容 |

### Stripe {#stripe}

支持国际信用卡/借记卡支付，适合面向海外用户的场景：

| 字段 | 说明 |
|------|------|
| Secret Key | Stripe 密钥（以 `sk_` 开头） |
| Webhook Secret | Webhook 签名密钥（以 `whsec_` 开头） |

#### Webhook 配置 {#stripe-webhook}

在 [Stripe Dashboard](https://dashboard.stripe.com/webhooks) 中添加 Webhook 端点：

1. 点击「添加端点」
2. 端点 URL 填写：`https://您的域名/api/callbacks/stripe`
3. 选择监听事件：`checkout.session.completed`
4. 创建后复制端点的「签名密钥」（以 `whsec_` 开头），填入 Novaix 的「Webhook Secret」字段

::: tip
如果您的业务涉及银行转账等异步支付方式，还需额外添加 `checkout.session.async_payment_succeeded` 事件。普通信用卡/借记卡支付只需 `checkout.session.completed` 即可。
:::

::: warning 最低金额限制
Stripe 对每笔交易有最低金额要求，具体取决于结算货币（如 USD 最低 $0.50、GBP 最低 £0.30）。跨币种支付时，Stripe 会按结算货币的最低额校验。如果充值金额低于最低限制，Stripe 会拒绝创建支付会话。请避免设置过低的套餐价格或充值金额。详见 [Stripe 最低金额说明](https://docs.stripe.com/currencies#minimum-and-maximum-charge-amounts)。
:::

### PayPal {#paypal}

支持 PayPal 国际支付：

| 字段 | 说明 |
|------|------|
| Client ID | PayPal 应用客户端 ID |
| Secret | PayPal 应用密钥 |
| Webhook ID | PayPal Webhook 端点 ID，用于验证回调签名 |
| 结算货币 | 支持 USD、EUR、GBP、JPY、CNY，默认 USD |
| 沙盒模式 | 是否使用沙盒环境（测试用） |

### 易支付 {#epay}

支持易支付通用接口规范（兼容彩虹易支付、ZPAY 等平台），适用于对接第三方聚合支付：

| 字段 | 说明 |
|------|------|
| 接口地址 | 易支付平台的 API 地址（无需包含 `/submit.php`） |
| 商户 ID | 商户编号 |
| 商户密钥 | MD5 签名密钥 |
| 支付方式 | 支持多选（支付宝、微信支付、QQ 钱包、网银、京东支付、PayPal、USDT），选择多种时用户下单时可自行选择 |
| 接口模式 | **页面跳转**（默认）或 **API 直接出码** |

易支付支持两种接口模式：

- **页面跳转**：用户点击支付后跳转到支付平台的收银台页面，选择支付方式完成支付
- **API 直接出码**（MAPI）：系统直接调用支付平台的 MAPI 接口获取二维码，在页面内展示给用户扫码支付，无需跳转。需要支付平台支持 MAPI 接口

::: tip
配置多种支付方式后，用户在充值或下单支付时会看到具体的支付方式选项（如"支付宝"、"微信支付"），而非笼统的"易支付"。如果只配置了一种支付方式，则直接以该方式的名称展示。
:::

支付完成后，系统通过异步回调自动更新订单状态。

## 充值设置 {#recharge}

在 **设置 → 充值** 中可以配置最小充值金额。系统默认最低充值 1 元，管理员可以设置更高的金额来抬高充值门槛。设为 0 表示使用系统默认值（1 元）。

## 手续费设置 {#payment-fee}

每个支付渠道启用后，可以独立配置手续费。开启手续费后，用户通过该渠道支付时会在原价基础上额外收取手续费，手续费明细会在支付前透明展示给用户。余额支付不涉及第三方渠道，不收取手续费。

在支付渠道的配置中，展开「手续费」区域即可设置：

| 字段 | 说明 |
|------|------|
| 手续费类型 | **不收取**（默认）、**固定金额**、**按比例** |
| 手续费金额 | 固定金额模式下单位为分（如 200 = 2 元）；按比例模式下为千分比（如 6 = 0.6%，29 = 2.9%，30 = 3%） |

配置示例：

- 固定金额：每笔支付加收 2 元 → 手续费类型选「固定金额」，金额填 `200`
- 按比例：每笔支付加收 0.6% → 手续费类型选「按比例」，金额填 `6`
- 按比例：每笔支付加收 2.9% → 手续费类型选「按比例」，金额填 `29`

用户在充值或支付订单时选择该渠道后，页面会显示手续费明细：

```
充值金额：¥100.00
手续费（0.6%）：+¥0.60
实付金额：¥100.60
```

::: tip
手续费仅影响用户实际支付的金额，不影响订单金额和余额到账金额。例如用户充值 100 元并支付 3 元手续费，最终到账余额为 100 元。
:::

## 支付流程 {#payment-flow}

下图展示了用户从下单到支付完成的完整流程：

```mermaid
sequenceDiagram
    participant U as 用户浏览器
    participant N as Novaix
    participant P as 支付渠道<br/>（支付宝/微信/Stripe 等）

    U->>N: 提交订单
    N->>N: 创建订单（待支付）
    N->>P: 调用支付接口<br/>（创建支付单）
    P-->>N: 返回支付链接/二维码
    N-->>U: 跳转支付页 / 展示二维码

    U->>P: 完成支付
    P->>N: 异步回调通知<br/>POST /api/callbacks/{渠道}
    N->>N: 验签 · 金额校验 · 幂等处理
    N->>N: 订单标记已支付
    N->>N: 按订单类型履行：<br/>新购→开通实例 / 续费→延长到期<br/>升级→升级任务 / 附加 IP→分配 IP<br/>流量包→入账或重置
    N-->>P: 返回 success
```

## 回调地址 {#callback-urls}

每个支付渠道都有对应的回调地址，用于接收支付结果通知。请确保您的反向代理正确转发了这些请求：

| 渠道 | 回调地址 |
|------|---------|
| 支付宝 | `https://您的域名/api/callbacks/alipay` |
| 微信支付 | `https://您的域名/api/callbacks/wechat` |
| Stripe | `https://您的域名/api/callbacks/stripe` |
| PayPal | `https://您的域名/api/callbacks/paypal` |
| 易支付 | `https://您的域名/api/callbacks/epay` |

::: warning
回调地址必须能从公网访问，且必须使用 HTTPS。如果支付后订单状态没有自动更新，请先检查回调地址是否可以正常访问。
:::
