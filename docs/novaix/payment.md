...
支持支付宝电脑网站支付，采用**证书模式**配置，需要准备以下信息：

| 字段 | 说明 |
|------|------|
| App ID | 支付宝开放平台应用 ID |
| 应用私钥 | RSA2 应用私钥 |
| 应用公钥证书 | 应用公钥证书内容（`appCertPublicKey_*.crt`） |
| 支付宝公钥证书 | 支付宝公钥证书内容（`alipayCertPublicKey_RSA2.crt`） |
| 支付宝根证书 | 支付宝根证书内容（`alipayRootCert.crt`） |
| 沙箱模式 | 是否使用沙箱环境（测试用） |

::: tip
您需要在[支付宝开放平台](https://open.alipay.com)创建应用，选择**公钥证书**方式签名，下载三个证书文件后将内容填入对应字段。
:::
...
| Secret | PayPal 应用密钥 |
| Webhook ID | PayPal Webhook 端点 ID，用于验证回调签名 |
| 结算货币 | 支持 USD、EUR、GBP、JPY、CNY，默认 USD |
| 沙盒模式 | 是否使用沙盒环境（测试用） |
...
| 商户 ID | 商户编号 |
| 商户密钥 | MD5 签名密钥 |
| 支付方式 | 支持多选（支付宝、微信支付、QQ 钱包、网银、京东支付、PayPal、USDT），选择多种时用户下单时可自行选择 |
...
::: tip
配置多种支付方式后，用户在充值或下单支付时会看到具体的支付方式选项（如"支付宝"、"微信支付"），而非笼统的"易支付"。如果只配置了一种支付方式，则直接以该方式的名称展示。
:::
...
在 **设置 → 充值** 中可以配置最小充值金额。系统默认最低充值 1 元，管理员可以设置更高的金额来抬高充值门槛。设为 0 表示使用系统默认值（1 元）。
...
sequenceDiagram
    ...
    N->>N: 验签 · 金额校验 · 幂等处理
    N->>N: 订单标记已支付
    N->>N: 按订单类型履行：<br/>新购→开通实例 / 续费→延长到期<br/>升级→升级任务 / 附加 IP→分配 IP<br/>流量包→入账或重置
    N-->>P: 返回 success
```
