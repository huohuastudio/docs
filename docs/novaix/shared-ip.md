...
- **端口映射**：根据套餐的[端口分配模式](./plan#nat-mode)，每个实例获得固定端口段或按需端口额度：
  - **固定端口段**：分配一段连续端口（如 `10000-10019`），首个端口固定映射到 SSH 端口 22，其余端口内外一致可自由使用
  - **按需选端口**：仅分配一个 SSH 端口，其余端口由用户通过[端口转发](./port-forward)按需添加，以额度数量为限

用户购买 NAT 套餐后会收到：
- 共享 IP 地址
- 分配的端口范围（固定端口段模式）或 SSH 端口和可选范围（按需模式）
- SSH 登录命令（如 `ssh -p 10000 root@203.0.113.1`）
- 如果套餐开启了「分配独立 IPv6」，还会收到一个独立的公网 IPv6 地址（如 `2001:db8::a`），可通过标准端口直接访问
...
graph TB
    ...
    subgraph node["节点服务器"]
        ...
        portMap -->|":10000-10019<br/>:10000 → 内部 :22"| vm1["实例 A<br/>10.0.0.2"]
        portMap -->|":10020-10039<br/>:10020 → 内部 :22"| vm2["实例 B<br/>10.0.0.3"]
        portMap -->|":10040-10059<br/>:10040 → 内部 :22"| vm3["实例 C<br/>10.0.0.4"]
    end
...
| 端口分配策略 | 「顺序」或「随机」，详见 [端口分配规则](#port-allocation) |
| 描述 | 备注信息（可选） |
...
系统会根据 [NAT 套餐](./plan#nat-mode) 中的端口分配模式，自动从共享 IP 的可用范围中分配端口资源。固定端口段模式分配连续端口块，按需模式仅分配 SSH 端口。每个共享 IP 可以独立选择端口分配策略：
...
- 端口号规律递增，方便运维通过端口号快速定位实例

### 随机分配

从可用的端口区间中随机选择一个端口块，块内端口仍然连续：

- 端口号不可预测，其他用户无法通过端口号推断系统规模或其他用户的端口
- 适合对端口隐蔽性有要求的场景

无论哪种策略，当一个共享 IP 的端口耗尽时，系统都会自动尝试该节点上的其他共享 IP。

::: tip
**容量估算**：端口范围 `10000-60000`（共 50001 个端口），固定端口段模式下每实例 20 个端口可容纳约 2500 个实例；按需模式下每实例仅占用 1 个 SSH 端口，同一共享 IP 可容纳更多实例。
:::
...
- **强制删除**：当节点不可达导致无法正常删除时，可以选择强制删除。强制删除会跳过端口分配检查，忽略远端转发规则清理失败，直接清理数据库记录
