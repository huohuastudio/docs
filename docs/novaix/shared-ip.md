...
- **端口映射**：每个实例获得一段连续端口（如 `10000-10019`），其中第一个端口固定映射到实例内部的 SSH 端口 22（即外网 `10000` → 内网 `22`），其余端口内外一致可自由使用
...
系统会根据 [NAT 套餐](./plan#nat-mode) 中配置的端口数量，自动从共享 IP 的可用范围中分配连续端口块。每个共享 IP 可以独立选择端口分配策略：
