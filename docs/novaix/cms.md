...
Novaix 内置了完整的 CMS 内容管理模块，为第三方主题提供动态数据支持。所有内容通过管理后台「内容管理」入口统一管理，大部分模块都提供公开 API 供主题渲染，无需认证即可访问。

::: warning 授权版功能
「团队成员」和「品牌素材」模块的公开 API 和后台管理功能仅授权版可用，免费版无法访问这两个模块的接口。
:::
...
轮播图按 `location` 分组（如 `homepage`、`sidebar`），支持设置展示时间范围（`start_at`、`end_at`）。公开 API 自动过滤未到时间或已过期的轮播图。
...
完整的 API 文档可在后台「高级设置」中开启「API 文档」后，访问 `/docs`（Scalar UI）查看。
