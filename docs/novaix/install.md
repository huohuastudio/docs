...
::: tip 最快上手路径
如果您希望尽快体验，推荐使用 **systemd + Caddy + SQLite（默认）** 的组合：下载二进制 → 修改配置 → systemd 托管 → Caddy 反向代理。整个过程 5 分钟内可以完成。
:::
...
此时您可以直接访问 `http://服务器IP:8080`，确认能看到登录页面，说明程序运行正常。确认后按 `Ctrl+C` 停止，继续完成以下配置。
