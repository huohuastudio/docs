...
VPC 功能依赖节点组的 OVN 支持和管理员启用。在创建 VPC 之前，请确保：

1. 系统已激活授权（VPC 为授权版功能）
2. 管理员在「系统设置」→「私有网络」中启用了 VPC 功能（默认关闭）
3. 已创建[节点组](./node-group)，且启用了 OVN
4. 节点组内至少有一个在线节点
5. 节点服务器上已正确配置 OVN 网络环境

::: tip
VPC 功能默认关闭，管理员手动开启后用户端才会显示「私有网络」入口。如果您的业务不需要 VPC，可以保持关闭以简化用户界面。
:::
...
- VPC CIDR 掩码不能小于 /29；子网 CIDR 掩码不能小于 /30
- VPC 创建后，CIDR 不可修改
:::
...
创建后系统会自动设置网关 IP（VPC 网段的 `.1` 地址，如 VPC 为 `10.10.0.0/16` 则网关为 `10.10.0.1`）和 DNS 服务器（默认 `8.8.8.8` / `8.8.4.4`）。

::: warning
- 同一 VPC 下的子网 CIDR 不能重叠
- 子网的网络地址（首个 IP）和广播地址（末尾 IP）会被保留；若 VPC 网关地址落在该子网范围内也会被保留，均不能分配给实例
- 删除子网前必须先卸载该子网下的所有实例
:::
...
| 协议 | TCP、UDP、ICMPv4、ICMPv6，留空表示所有协议 |
| 来源/目标 | IP 地址或网段 |
| 端口范围 | 端口或端口范围，如 `80,443` 或 `8000-9000` |

规则创建后会自动同步到所有关联实例，通常几秒内生效。如需调整已有规则，请删除后重新创建。
...
删除 VPC 前必须先删除其下的所有子网（子网删除前需卸载所有实例）。安全组和安全规则会随 VPC 删除自动清理，无需手动删除。

::: warning
删除 VPC 会同时清理节点上对应的 OVN 网络资源和所有关联的安全组，此操作不可恢复。
:::
