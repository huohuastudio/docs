...
## Docker 安装 <Badge type="tip" text="推荐" />

::: tip
兰空图床现已支持在[宝塔](https://www.bt.cn) Docker 环境中一键安装。打开宝塔 Docker 应用商店，搜索 `lsky pro`。名称为 `Lsky Pro+` 的是付费版本。
:::
...
::: warning
首次运行会在此目录生成 `.env` 文件，请保持为空文件，启动成功后初次访问安装页面，系统检测到是空的文件才会写入正确的内容，否则直接应用此 `.env` 文件。这意味着你可以完全可以通过 `~/data/.env` 文件自定义环境变量。
:::
